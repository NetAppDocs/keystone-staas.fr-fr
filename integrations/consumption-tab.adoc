...
+
Le graphique linéaire affiche les données historiques de consommation et permet aux utilisateurs d'analyser les tendances sur une plage de dates sélectionnée. Le graphique présente des indicateurs tels que la capacité utilisée (capacité de stockage totale, incluant à la fois les données et les données d'instantané), les données (capacité de stockage utilisée par les données utilisateur) et l'instantané (capacité de stockage utilisée par les données d'instantané), ainsi que la capacité engagée et la capacité de pointe. En fonction de la plage de dates de votre requête, les graphiques d'utilisation affichent jusqu'à 30 points de collecte de données. Vous pouvez survoler le graphique avec le curseur de votre souris pour afficher la répartition de l'utilisation à chaque point de collecte de données.
