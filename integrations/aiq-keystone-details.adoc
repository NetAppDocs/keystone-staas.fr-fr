...
TIP: L'info-bulle en regard de l'ID de suivi de votre abonnement dans l'onglet *Détails de l'abonnement* vous aide à identifier l'abonnement partenaire dans la configuration MetroCluster.
...
L'image suivante affiche les graphiques pour le niveau de service _Extreme_ (niveau de service de base) et un numéro d'abonnement principal. Le même tableau de séries chronologiques indique également la consommation du site miroir dans une nuance plus claire du même code de couleur que celui utilisé pour le site principal. L'info-bulle affichée avec le curseur de la souris affiche l'éclatement de la consommation (en Tio) pour les sites principaux et en miroir, 1.02 Tio et 1.05 Tio respectivement.
